...
for (k in 1:n_folds) {
  ...
  rf.AD <- randomForest(DX_bl ~ ., data = data.train, ntree = 200)  # Fit the linear model with the training data
  y_hat <- predict(rf.AD, data.test,type="class")     # Predict on the testing data using the trained model
  true_y <- data.test$DX_bl                  # get the true y values for the testing data
