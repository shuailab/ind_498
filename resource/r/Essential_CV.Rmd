...
```{r}
# Step 2 -> gather a list of candidate models
...
```{r}
# Use 10-fold cross-validation to select the best linear regression model
...
```{r}
# Build a decision tree model using the r package rpart. Use 10-fold cross-validation to select the best tree size. Get the 10-fold cross-validation error.
...
# Note that, as we did in linear regression model, we need to specify a list of candidate models (usually from simple to complex), then use 10-fold cross-validation to evaluate each of them. Then we pick up the best model with minimum prediciton error. Here, as the complexity of a decision tree is largely determined by the depth (also directly related to the minimum # of samples in the leaf nodes), we can evaluate a list of candidate models by trying a tree with minbucket = 2, a tree with minbucket = 4, a tree with minbucket = 6, and a tree with minbucket = 8 (this model only has one root node, thus, it is the simplest model without any split)
